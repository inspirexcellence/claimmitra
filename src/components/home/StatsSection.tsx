  ...
  return (
    <section className="relative py-16 bg-slate-50">
      <div className="container mx-auto px-4 relative z-10">
        <div className="bg-gradient-to-br from-slate-900 via-[#0B1A2C] to-blue-950 rounded-[2.5rem] shadow-2xl p-8 lg:p-14 overflow-hidden relative">
          
          {/* Dynamic background glow */}
          <div className="absolute top-0 right-0 w-96 h-96 bg-blue-600/20 rounded-full blur-[100px] translate-x-1/3 -translate-y-1/3 pointer-events-none" />
          ...
          <StaggerContainer className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 text-center relative z-10">
            {stats.map((stat, i) => (
              <StaggerItem key={i} className="flex flex-col items-center justify-center p-6 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm hover:bg-white/10 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/20 group">
                ...
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-blue-500/20 flex items-center justify-center mb-4 md:mb-5 group-hover:scale-110 group-hover:bg-blue-500/30 transition-all duration-300">
                  <stat.icon className="w-6 h-6 md:w-7 md:h-7 text-blue-400 group-hover:text-blue-300 transition-colors" />
                </div>
                ...
                <p className="text-blue-200/80 font-bold text-[10px] md:text-xs lg:text-sm uppercase tracking-widest mt-1">
                  {stat.label}
                </p>
