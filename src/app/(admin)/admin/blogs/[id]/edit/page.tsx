...
export default function EditBlogPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    fetchBlog();
  }, [unwrappedParams.id, reset, router]);
  ...
  const onSubmit = async (data: BlogFormValues) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/blogs/${unwrappedParams.id}`, { 
        method: "PUT", 
        body: formData 
      });
      ...
      toast.success(data.published ? "Blog post updated and published!" : "Draft updated successfully!");
      router.push("/admin/blogs");
    } catch (error: any) {
  ...
  if (isLoading) return <div className="p-8">Loading blog data...</div>;
    ...
    <div>
      <div className="mb-6 flex flex-col space-y-4">
        ...
        <div>
          ...
          <p className="text-slate-500 mt-1">Update your article using the rich text editor.</p>
        </div>
      </div>
          ...
          <div className="space-y-6">
            <Card>
              <CardContent className="p-6 space-y-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="image">Update Featured Image (Optional)</Label>
